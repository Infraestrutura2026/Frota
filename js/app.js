...
const App = (function() {
  ...
  async function loadData() {
    if (CONFIG.isConfigured) {
      // Tenta leitura primeiro — se funcionar, assume online
  ...
  function renderVehicles() {
    ...
    tbody.innerHTML = filtered.map(v => `
      <tr>
        <td>${v.foto ? '<img src="' + v.foto + '" class="vehicle-thumb" alt="">' : '<div class="vehicle-thumb-placeholder">🚗</div>'}</td>
        <td><b>${v.placa}</b></td>
        <td>${v.grupo || '-'}</td>
    ...
    `).join('') || '<tr><td colspan="9" class="empty-state"><div class="empty-icon">🚗</div><h4>Nenhum veículo encontrado</h4></td></tr>';
  }
  ...
  // ========== FOTO DO VEÍCULO ==========
  function resizeImage(file, maxWidth = 400, quality = 0.7) {
  ...
  function updateFotoPreview(src) {
    ...
    if (src) {
      preview.innerHTML = '<img src="' + src + '" alt="Foto do veículo">';
      document.getElementById('v-foto').value = src;
    } else {
      preview.innerHTML = '<span style="color:#94a3b8;font-size:12px;">📷 Sem foto</span>';
      document.getElementById('v-foto').value = '';
    }
  }

  function bindFotoUpload() {
    const input = document.getElementById('v-foto-input');
    if (!input) return;
  ...
  function gerarRelatorio() {
    ...
    switch (reportAtual) {
      ...
      case 'emManutencao': {
        headers = ['Placa', 'Modelo', 'Serviço', 'Data Início', 'Previsão', 'Status'];
        const hoje = new Date().toISOString().slice(0, 10);
        rows = maintenance.filter(x => x.status !== 'Concluído' && (!placa || x.placa === placa)).map(x => {
          const v = vehicles.find(v => v.placa === x.placa) || {};
          const status = x.previsao < hoje ? '🔴 Atrasado' : '🟢 Em andamento';
          return [x.placa, v.modelo || '-', x.servico, x.data, x.previsao, status];
        });
        break;
      }
      case 'parados': {
        headers = ['Placa', 'Modelo', 'Último KM', 'Data Última Viagem', 'Dias Parado'];
        const hoje = new Date();
        rows = vFilt.map(v => {
          const ult = km.filter(k => k.placa === v.placa).sort((a, b) => (b.data || '').localeCompare(a.data || ''))[0];
          const dias = ult && ult.data ? Math.ceil((hoje - new Date(ult.data)) / (1000 * 60 * 60 * 24)) : 9999;
          return [v.placa, v.modelo, ult ? ult.km_atual : '-', ult ? ult.data : '-', dias > 9990 ? 'Nunca usado' : dias + ' dias'];
        }).filter(r => r[4] !== 'Nunca usado' && parseInt(r[4]) > 7).sort((a, b) => parseInt(b[4]) - parseInt(a[4]));
        break;
      }
      case 'kmMotorista': {
